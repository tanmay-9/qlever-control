"""Parsing of qlever-server log lines into METRIC events."""
...
def is_timestamped(line: bytes) -> bool:
    """Cheap structural check: line begins with 'YYYY-MM-DD HH:...'."""
    return (
        len(line) >= TS_LEN
        and line[4] == 0x2D  # '-'
        and line[7] == 0x2D  # '-'
...
def extract_timestamp(line: bytes) -> bytes:
    """Return the 23-byte timestamp prefix, suitable for lexical comparison."""
    return line[:TS_LEN]


def format_log_timestamp(dt: datetime) -> bytes:
    """Render a datetime as a 23-byte log-format timestamp."""
    # strftime gives 6-digit microseconds; truncate to milliseconds.
...
def read_line_after(
    ...
    """Seek near probe_offset and return (timestamp_bytes, line_start_offset)
    of the next timestamped line, or None at EOF.
    """
...
def read_first_timestamp(log: BinaryIO) -> datetime:
    """Read the first timestamped line's timestamp by scanning forward from
...
def read_last_timestamp(log: BinaryIO, file_size: int) -> datetime:
    """Read the last timestamped line's timestamp, growing the tail buffer
    until one is found.
    """
    tail = 32 * 1024
    while True:
...
def find_offset_in_window(
    ...
    """Return a byte offset whose line timestamp lies in [oldest_target,
    newest_target] (lexical bytes). Falls back to an offset older than
    oldest_target when no in-window sample is found, so callers always
    over-include rather than under-include.
    """
    log.seek(0, os.SEEK_END)
    file_size = log.tell()

    # Phase 1: gallop backward from EOF until we land in-window or overshoot.
...
def parse_metric_line(line: bytes) -> dict | None:
    """Parse a single log line as a METRIC event, or None if it isn't one."""
...
def group_queries(
    ...
    """Group events by lifecycle: returns (active_by_qid, completed_pairs).
    ...
    Orphan end events (start outside the window) are dropped.
    """
    active = {}
    completed = []
    for event in events:
...
def get_historic_window(
    ...
    """Return (active, completed, last_scanned_ms) for [start_dt, end_dt].

    Scans an extended range [start_dt - 2*timeout, end_dt + 2*timeout] so
    that queries whose start precedes the window or whose end follows it
    are still captured. 'active' are starts whose end was never seen even
    in the extended scan. last_scanned_ms is the largest event timestamp
    encountered; it bounds how far our 'no end seen' claim is verified.
    """
    ...
    overlapping_completed = [
        ...
        if s["started-at"] <= end_ms and e["ended-at"] >= start_ms
    ]
    overlapping_active = [
        s for s in active_by_qid.values() if s["started-at"] <= end_ms
    ]
    return overlapping_active, overlapping_completed, last_scanned_ms


def read_log_bounds(log_file: Path) -> tuple[datetime, datetime]:
    """Return (first_ts, last_ts) datetimes for the log file."""
    with log_file.open("rb") as log:
        log.seek(0, os.SEEK_END)
        file_size = log.tell()
...
def get_live_active_queries(
    ...
    """Return start events for queries that began but did not end within
    the last ~2*timeout near EOF.
    """
    with log_file.open("rb") as log:
        log.seek(0, os.SEEK_END)
        file_size = log.tell()
